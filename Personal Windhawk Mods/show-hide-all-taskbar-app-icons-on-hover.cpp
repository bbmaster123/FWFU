...
// @version         1.0.0
// @author          Preston Percival (modified by bbmaster123)
...
This mod allows you to adjust the opacity of all application icons on the Windows 11 taskbar at once. Tray area not affected
...
auto onEnter = [weak](winrt::Windows::Foundation::IInspectable const& sender, PointerRoutedEventArgs const&) {
    // Loop through ALL registered buttons and set opacity to 1.0
    std::lock_guard<std::mutex> lock(g_buttonsMutex);
    for (auto& ref : g_buttonRefs) {
        if (auto strong = ref.get()) {
            strong.Opacity(1.0);
        }
    }
};
