...
- barNormalEnd: "#27AE60"
  $name: Bar Color End
- barFullStart: "#E74C3C"
  $name: Bar Full Color Start
- barFullEnd: "#C0392B"
  $name: Bar Full Color End
- trackColor: "#20000000"
...
- showGloss: true
  $name: Bar Enable Glossy Overlay
- leftInset: 0
  $name: Fill Inset Left
- rightInset: 0
  $name: Fill Inset Right
- topInset: 0
  $name: Fill Inset Top
- bottomInset: 0
  $name: Fill Inset Bottom
- formatString: "%s free | %s used of %s"
  $name: Text Display Format
- boldUsed: true
  $name: Text Bold Used Space
- boldStyle: sans-serif
  $name: Text Bold Style
  $options:
    - serif: Serif Bold
    - sans-serif: Sans-Serif Bold
- removeSpace: false
  $name: Text Remove Space from Units
- lineYOffset: 0
...
void LoadSettings() {
    PCWSTR s;
    s = Wh_GetStringSetting(L"barNormalStart"); g_barNormalStart = ParseHexARGB(s, 0xFF2ECC71); Wh_FreeStringSetting(s);
    s = Wh_GetStringSetting(L"barNormalEnd");   g_barNormalEnd   = ParseHexARGB(s, 0xFF27AE60); Wh_FreeStringSetting(s);
    s = Wh_GetStringSetting(L"barFullStart");   g_barFullStart   = ParseHexARGB(s, 0xFFE74C3C); Wh_FreeStringSetting(s);
    s = Wh_GetStringSetting(L"barFullEnd");     g_barFullEnd     = ParseHexARGB(s, 0xFFC0392B); Wh_FreeStringSetting(s);
    s = Wh_GetStringSetting(L"trackColor");     g_trackColor     = ParseHexARGB(s, 0x20000000); Wh_FreeStringSetting(s);
    s = Wh_GetStringSetting(L"borderColor");    g_borderColor    = ParseHexARGB(s, 0x80FFFFFF); Wh_FreeStringSetting(s);
...
std::wstring CleanNumericString(const std::wstring& s) {
    ...
    for (wchar_t c : s) { 
        ...
        if (!start) { if (iswdigit(check) || check == L'.' || check == L',' || check == L'-') { start = true; result += check; } continue; } 
...
std::wstring MakeBoldText(const std::wstring& s) {
    ...
    for (wchar_t c : s) {
        ...
        else if (c >= L'A' && c <= L'Z') { res += (wchar_t)0xD835; res += (wchar_t)((g_boldStyle == BoldStyle::Serif ? 0xDC00 : 0xDDD4) + (c - L'A')); }
        else if (c >= L'a' && c <= L'z') { res += (wchar_t)0xD835; res += (wchar_t)((g_boldStyle == BoldStyle::Serif ? 0xDC1A : 0xDDEE) + (c - L'a')); }
        else res += c;
    }
    return res;
}
...
static void BuildRoundedPath(GraphicsPath& path, RectF rect, float radius) {
    ...
    if (d < 1.0f) { path.AddRectangle(rect); return; }
...
int WINAPI DrawTextW_Hook(HDC hdc, LPCWSTR psz, int cch, LPRECT prc, UINT fmt) {
    ...
    if (FindSpaceStats(t, fs, ts)) {
        if (g_removeSpace) { fs.erase(std::remove(fs.begin(), fs.end(), L' '), fs.end()); ts.erase(std::remove(ts.begin(), ts.end(), L' '), ts.end()); }
        ...
        if (swscanf(cf.c_str(), L"%lf %15s", &fv, fu) == 2 && swscanf(ct.c_str(), L"%lf %15s", &tv, tu) == 2) {
            ...
            if (fmt & DT_CALCRECT) { std::wstring ft = s1 + s2 + s3; return DrawTextW_Orig(hdc, ft.c_str(), (int)ft.length(), prc, fmt); }
            UINT dF = (fmt & ~(DT_END_ELLIPSIS | DT_PATH_ELLIPSIS | DT_WORD_ELLIPSIS)) | DT_LEFT | DT_SINGLELINE | DT_NOPREFIX | DT_NOCLIP | DT_TOP;
            SIZE sz1, sz2, sz3; GetTextExtentPoint32W(hdc, s1.c_str(), (int)s1.length(), &sz1); GetTextExtentPoint32W(hdc, s2.c_str(), (int)s2.length(), &sz2); GetTextExtentPoint32W(hdc, s3.c_str(), (int)s3.length(), &sz3);
            int x = prc->left; if (fmt & DT_CENTER) x += (prc->right - prc->left - (sz1.cx + sz2.cx + sz3.cx)) / 2; else if (fmt & DT_RIGHT) x = prc->right - (sz1.cx + sz2.cx + sz3.cx);
            RECT r = *prc; r.top += g_lineYOffset; r.bottom += g_lineYOffset; r.left = x; r.right = x + sz1.cx; DrawTextW_Orig(hdc, s1.c_str(), (int)s1.length(), &r, dF);
            r.left = r.right; r.right = r.left + sz2.cx; r.top += g_boldYOffset; r.bottom += g_boldYOffset; DrawTextW_Orig(hdc, s2.c_str(), (int)s2.length(), &r, dF);
            r.left = r.right; r.right = r.left + sz3.cx; r.top -= g_boldYOffset; r.bottom -= g_boldYOffset; return DrawTextW_Orig(hdc, s3.c_str(), (int)s3.length(), &r, dF);
        }
    }
...
int WINAPI DrawTextExW_Hook(HDC hdc, LPWSTR psz, int cch, LPRECT prc, UINT fmt, LPDRAWTEXTPARAMS pDtp) {
    ...
    if (FindSpaceStats(t, fs, ts)) {
        ...
        if (swscanf(cf.c_str(), L"%lf %15s", &fv, fu) == 2 && swscanf(ct.c_str(), L"%lf %15s", &tv, tu) == 2) {
            ...
            std::wstring fS = g_formatString; size_t p1 = fS.find(L"%s"), p2 = fS.find(L"%s", p1+2), p3 = fS.find(L"%s", p2+2);
            std::wstring s1 = fS.substr(0, p1) + fs + fS.substr(p1+2, p2-p1-2), s2 = g_boldUsed ? MakeBoldText(us) : us, s3 = fS.substr(p2+2, p3-p2-2) + ts + fS.substr(p3+2);
            if (fmt & DT_CALCRECT) { std::wstring ft = s1 + s2 + s3; std::vector<wchar_t> b(ft.begin(), ft.end()); b.push_back(0); return DrawTextExW_Orig(hdc, b.data(), (int)ft.length(), prc, fmt, pDtp); }
            SIZE sz1, sz2, sz3; GetTextExtentPoint32W(hdc, s1.c_str(), (int)s1.length(), &sz1); GetTextExtentPoint32W(hdc, s2.c_str(), (int)s2.length(), &sz2); GetTextExtentPoint32W(hdc, s3.c_str(), (int)s3.length(), &sz3);
            int x = prc->left; if (fmt & DT_CENTER) x += (prc->right - prc->left - (sz1.cx + sz2.cx + sz3.cx)) / 2;
            RECT r = *prc; r.top += g_lineYOffset; r.bottom += g_lineYOffset; r.left = x; r.right = x + sz1.cx;
            std::vector<wchar_t> b1(s1.begin(), s1.end()); b1.push_back(0); DrawTextExW_Orig(hdc, b1.data(), (int)s1.length(), &r, fmt | DT_NOPREFIX | DT_LEFT, pDtp);
            r.left = r.right; r.right = r.left + sz2.cx; r.top += g_boldYOffset; r.bottom += g_boldYOffset;
            std::vector<wchar_t> b2(s2.begin(), s2.end()); b2.push_back(0); DrawTextExW_Orig(hdc, b2.data(), (int)s2.length(), &r, fmt | DT_NOPREFIX | DT_LEFT, pDtp);
            r.left = r.right; r.right = r.left + sz3.cx; r.top -= g_boldYOffset; r.bottom -= g_boldYOffset;
            std::vector<wchar_t> b3(s3.begin(), s3.end()); b3.push_back(0); return DrawTextExW_Orig(hdc, b3.data(), (int)s3.length(), &r, fmt | DT_NOPREFIX | DT_LEFT, pDtp);
        }
    }
